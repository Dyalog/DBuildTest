...
⍝ this is only possible on Windows (APLProcess.GetExitCode only usable on Win)
 ...
 :If ##._isWin
     ...
     myapl←⎕NEW #.APLProcess(''('ok=0 LX="{2 ⎕nq ⎕se ''KeyPress'' ⍵}¨'']dtest ',##.TESTSOURCE,'test_retcode.dyalog -off'',⊂''ER''"'))
     ...
     myapl←⎕NEW #.APLProcess(''('ok=1 LX="{2 ⎕nq ⎕se ''KeyPress'' ⍵}¨'']dtest ',##.TESTSOURCE,'test_retcode.dyalog -off'',⊂''ER''"'))
     :While ~myapl.HasExited ⋄ :EndWhile ⍝ let it run...
