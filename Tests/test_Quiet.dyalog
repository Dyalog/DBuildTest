 ...
 r←⍬
   ...
   ⍝ 1. For DTest
 rc←300 sub_RunAPLProcess((##.TESTSOURCE,'lx')('CITATest=',##.TESTSOURCE,'test_retcode.dyalog mode=DTest ok=1 dtestmods="-off -q -loglvl=40',(##.halt/' -halt'),' -testlog=',##.TESTSOURCE,'test_retcodeq"'))
 :If 0 Check 2 ⎕NINFO ##.TESTSOURCE,'test_retcode.session.log'
    ...
    ⍝ 2. for DBuild
 rc←300 sub_RunAPLProcess((##.TESTSOURCE,'lx')('RunUCMD="DBuild ',##.TESTSOURCE,'DBuild_1.dyalogbuild -q" TESTLOG="',##.TESTSOURCE,'test_dbuilq"'))
 :If 0 Check 2 ⎕NINFO ##.TESTSOURCE,'test_retcode.session.log'
     →0 Because'Running DTest with -q (="quiet") modifier produced output!' ⋄ :EndIf
